...
const StatCard = memo(({ value, label, delay = 0 }: { value: string; label: string; delay?: number }) => (
  <div 
    className="glass-card p-4 text-center hover-lift card-glow animate-slide-up opacity-0"
...
const FeatureBadge = memo(({ icon: Icon, text, iconColor, delay = 0 }: { 
  ...
  <Badge 
    variant="secondary" 
    className="px-3 py-1.5 text-xs sm:text-sm hover-lift animate-scale-in opacity-0"
    style={{ animationDelay: `${delay}ms`, animationFillMode: 'forwards' }}
  >
...
const Liquidity = () => {
  return (
    <main className="container py-4 sm:py-6 md:py-10 px-3 sm:px-4 relative">
      ...
      {/* Liquidity Panel with enhanced styling */}
